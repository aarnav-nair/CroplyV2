...
export default function Navbar({ cartCount, onNavigate, currentView, lang, setLang, theme, toggleTheme }) {
  ...
  return (
    <nav style={{background:'rgba(20,26,16,0.97)', backdropFilter:'blur(12px)'}}
      ...
      <div className="container h-[62px] flex items-center justify-between gap-6">
        ...
        <button onClick={() => onNavigate('home')}
          ...
          <div>
            ...
            <span className="text-[10px] font-body leading-none block mt-0.5"
      ...
      {/* Mobile menu */}
      {open && (
        <div style={{background:'rgba(14,20,12,0.98)'}}
          ...
          <button onClick={() => { onNavigate('scan'); setOpen(false) }}
                  style={{background:'var(--gold)',color:'var(--dark)'}}
